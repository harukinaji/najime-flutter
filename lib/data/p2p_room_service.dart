...
class P2PRoomService {
  ...
  Future<bool> joinMatchmaking() async {
    ...
    if (kDebugMode)
      debugPrint(
        '[P2P] matchmaking ok room=$roomId players=$players selfId=$selfId',
      );
    for (final pid in players) {
      if (pid != selfId && pid.isNotEmpty) {
        unawaited(_connectTo(pid));
  ...
  void _onWsPeerJoin(dynamic msg) {
    ...
    if (kDebugMode)
      debugPrint(
        '[P2P] ws peer_join peerId=$peerId room=$_roomId selfId=$selfId',
      );
  ...
  void _removePeer(String peerId, {String reason = 'unknown'}) {
    ...
    if (kDebugMode)
      debugPrint(
        '[P2P] removePeer $peerId reason=$reason peersLeft=${_peers.length}',
      );
    if (peer == null) return;
    peer.removalTimer?.cancel();
    peer.removalTimer = null;
  ...
  Future<bool> joinVoice() async {
    ...
    try {
      ...
      if (!mic.isGranted) {
        if (kDebugMode)
          debugPrint('[P2P] joinVoice: mic permission denied: $mic');
        return false;
      }
